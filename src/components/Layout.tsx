  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col">
      ...
      <header className="bg-white border-b border-zinc-200 sticky top-0 z-10">
        <div className="w-full px-3 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <img src="logo.svg" alt="Logo" className="h-7 w-auto shrink-0" />
            <h1 className="text-base font-bold text-zinc-900 tracking-tight hidden sm:block">EquipManager</h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setShowProfile(true)}
              className="sm:hidden p-2 text-zinc-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
              title="Meu Perfil"
            >
          ...
          <nav className="flex overflow-x-auto scrollbar-hide gap-0">
            {tabs.map(({ id, label, icon: Icon }) => (
              ...
              >
                ...
                {/* Mobile: only icon for non-active, icon+label for active */}
